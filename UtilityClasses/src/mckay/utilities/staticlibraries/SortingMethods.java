...
{
     /**
      * Takes the tree with the given root and reorders it so that all siblings
      * are sorted alphebetically. Recurses through the children so that each
      * level is sorted alphabetically. Returns the root of the new sorted tree.
      *
      ...
      *
      * @param	original_strings   The array to be sorted.
      * @return                    A sorted copy of original_strings.
      */
     public static String[] sortArray(String[] original_strings)
     {
          ...
          for (int i = 0; i < original_strings.length - 1; i++)
          ...
          for (int i = 0; i < original_strings.length; i++)
          {
               ...
               for (int j = 0; j < original_strings.length; j++)
               {
                    if(!added_already[j])
                    {
                         ...
                         else if (string_comparer.compare(earliest_yet, original_strings[j]) > 0)
                         {
                              earliest_yet = original_strings[j];
                              earliest_yet_index = j;
                         }
          ...
          for (int i = 0; i < original_strings.length; i++)
               sorted[i] = original_strings[sorted_ordering[i]];
      ...
      * @param	sorted_ordering    The number position assigned to each row 
      *                            after sorting.
     ...
     /**
      * Takes in an array of ints and returns a copy of the array sorted
      * from lowest to highest. The orignal array is not altered.
      *
      * @param	to_sort  The data that is to be sorted.
      * @return          A copy of the array sorted from lowest to highest.
