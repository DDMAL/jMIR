...
/**
 * <code>JFrame</code> used to select encoding options for recording or
 ...
 *
 * <p>It is not necessary to press the <i>OK</i> button in order for changes to
 * be accessible to external classes. However, pressing the <i>Cancel</i> button
 * will restore settings to those that were selected when the
 * <code>JFrame</code> was last made visible.
 *
 * <p>The GUI settings may be set or accessed externally by the
 * <code>setAudioFormat</code> and <code>getAudioFormat</code> methods
 * respectively.
 *
 * <p>This class also includes several static methods that are unrelated to the
 * GUI settings. These include the
 * <code>getStandardLowQualityRecordAudioFormat</code>,
 ...
 * similar names. The <code>defineAudioFormat</code> static method does the same
 * thing as the basic PCM <code>AudioFormat</code> constructor, but is better
 * documented.
 *
 * @author Cory McKay
 */
     ...
     implements ActionListener
{
     ...
     /**
      * Temporarily holds the AudioFormat corresponding to the GUI's settings
      * whenever the setVisible method is called. For use in restoring settings
     ...
     /**
      * Basic constructor. Configures the panel and its fields to low quality
      * audio. Prepares the <code>JFrame</code>, but does not show it. The
      * <code>setVisible</code> method must be called externally to show this.
      */
     ...
     /**
      * Returns a new mono <code>AudioFormat</code> that uses an 44.1 kHz
     ...
     /**
      * Returns a new <code>AudioFormat</code> with the given parameters. This
      * object describes the particular arrangement of data in a sound stream.
      *
      * <p>Linear PCM encoding is used automatically. An alternative
      * constructory of <code>AudioFormat</code> can be used if a different
      * encoding is desired.
      *
      * <p>This method does not do anything that a basic
      * <code>AudioFormat</code> constructor does not already do. The purpose of
      * this method is to give better documentation.
      *
      * <p>The possible parameters given below may varay from sound card to
      * sound card, and others may be available as well.
      *
      * @param	sample_rate   Number of samples per second. Standard values
      *                       are 8000,11025,16000,22050 or 44100.
      * @param	sample_size   Number of bits per sample. Standard values are 8
      *                       or 16.
      ...
      * @return               A linear PCM encoded <code>AudioFormat</code> with
      ...
      *
      * @param	audio_format  The <code>AudioFormat</code> to base GUI values
      *                       on.
      */
     ...
     /**
      * Gets the <code>AudioFormat</code> corresponding to the settings on the
      * GUI. Note that only PCM encoding is possible.
      *
      * @param	allow_text_selections   If this is not set to true, then this
     ...
     /**
      * Makes this <code>JFrame</code> visible or hidden, exactly as the
      * inherited <code>setVisible</code> method does. Also temporarily stores
     ...
     /**
      * Hides the <code>JFrame</code> and restores the GUI settings to those
      * that were selected when it was last made visible.
      */
     private void cancel()
     {
