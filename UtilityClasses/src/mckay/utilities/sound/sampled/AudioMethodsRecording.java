...
public class AudioMethodsRecording
{
     ...
     /**
      * Records the samples coming in on the given <code>TargetDataLine</code> 
      * to a <code>ByteArrayOutputStream</code>. Returns the thread that can be
      * stopped and from which the samples can be extracted. Useful for 
      * real-time recording.
      *
     ...
     /**
      * Records the samples coming in on the given <code>TargetDataLine</code> 
      * to the given file. Returns the thread that can be stopped when recording
      * is complete. Useful for real-time recording.
      *
      ...
      * @return                    The thread that performs the recording.
      * @throws	Exception          Throws an exception if the target_data_line
      *                            parameter is null or if the given file cannot 
      *                            be written to.
      */
      ...
      * to fill up and eventually use up all available memory if the data being 
      * written to it is not dealt with. This could result in an 
      * <code>OutOfMemory</code> error.
      *
      * <p><b>WARNING:</b>Note that the <code>TargetDataLine</code> is stopped 
      * and closed after recording has terminated. It will need to be re-started
          ...
          extends Thread
     {
           ...
           * <code>ByteArrayOutputStream</code>. Continues until the
           * <code>stopRecording</code> method is called.
           *
           * <p><b>WARNING:</b>Note that the <code>TargetDataLine</code> is 
           * stopped and closed after recording has terminated. It will need to
           * be re-started if it is to be used elsewhere.
           */
          public void run()
          {
               stop_recording = false;
           ...
           *
           * <p><b>WARNING:</b>Note that the <code>TargetDataLine</code> is 
           * stopped after recording has terminated. It will need to be 
           * re-started if it is to be used elsewhere.
           */
     ...
     /**
      * A thread for recording from a given <code>TargetDataLine</code> into an 
      * audio file belonging to one of a number of types. Can be interrupted by
      * using the <code>stopRecording</code> method.
      *
      * <p><b>WARNING:</b>Note that the <code>TargetDataLine</code> is stopped
      * and closed after recording has terminated. It will need to be re-started
      * if it is to be used elsewhere.
          ...
          extends Thread
     {
          ...
          /**
           * Constructor that performs standard operations of the 
           * <code>Thread</code> class as well as setting the source
           * <code>TargetDataLine</code> from which the audio to be recorded
           * will arrive, setting the audio file to record to and its type and 
           * setting the thread to be logically ready to record.
           *
           * @param      target_data_line    The source of the audio to be
           *                                 recorded.
           * @param      file_to_save_to     The file to save the audio to.
           * @param      file_type           The type of audio file to save to.
           * @throws     Exception           Throws an exception if the
           *                                 target_data_line parameter is null 
           *                                 or if the given file cannot be 
           *                                 written to.
           */
          ...
          /**
           * Causes the recording currently in progress to end and saves the 
