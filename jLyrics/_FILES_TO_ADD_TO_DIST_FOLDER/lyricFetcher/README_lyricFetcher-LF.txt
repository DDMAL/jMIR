...
===ABOUT===
This program constructs queries to LyricsFly <http://lyricsfly.com/>, each composed of an artist and song name. The target page is parsed and the lyrics (if any) are saved as a plaintext file. The lyrics are rudimentarily cleaned by removing extra spaces, deleting lines that begin with stopwords (such as 'verse' and 'bridge') and copying the text of choruses whenever the word 'chorus' is used as a placeholder.

===IMPORTANT===
To use this script, you must obtain individually obtain a permanent API key from LyricsFly. Note that their requirements to obtain one are strict. Details are available at their website: <http://lyricsfly.com/api/>
